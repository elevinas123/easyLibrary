...
import {extractArrayType, extractInnerType, extractUnionTypes,} from './extractType';
...
export type TypeDict = {
  [key: string]: {
    ...
    properties?: {name: string; type: string;
                  fullType: string}[];  // Only for interfaces
...
const fillDoc = (
    doc: DocEntry[], currentType: DocEntry, typeDict: TypeDict,
  ...
  if (currentType.properties) {
    ...
    const processedProperties = currentType.properties.map((property) => {
      ...
      unionTypes.forEach((ut) => {
        if (/^".*"$/.test(ut)) {
          ...
          if (referencedType) {
            ...
            typeDict = fillDoc(doc, referencedType, typeDict, processedTypes);
          } else {
            console.warn(`Referenced type "${ut}" not found in documentation.`);
          }
          typeStrings.push(ut);
        }
      });
      ...
      return {
        name: property.name,
        type: isArray ? `${innerType}[]` :
            unionTypes.length > 1 ? `(${typeStrings.join(' | ')})` :
                                    innerType,
...
const generateAllDocumentation = async () => {
  ...
  const pattern =
      path.join(sourceDir, '**', '*.ts');  // Include all TypeScript files
