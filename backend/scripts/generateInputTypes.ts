...
export const generateInputTypes =
    (inputMap: InputMapping, endPointMap: EndpointMapping) => {
      ...
      const outputPath =
          path.join(frontendDirPath, 'src', 'endPointTypes', 'inputMap.ts');
      ...
      function extractTypeNames(typeStr: string): string[] {
        ...
        const splitTypes =
            cleanedTypeStr.split(/[\|\&]/).map((type) => type.trim());

        splitTypes.forEach((type) => {
          // Remove generic wrappers (e.g., Partial<CreateBookDto> =>
          // CreateBookDto)
      ...
      if (imports.size > 0) {
        importStatements = `import { ${
            Array.from(imports).join(', ')} } from '${typesModulePath}';\n\n`;
      }
