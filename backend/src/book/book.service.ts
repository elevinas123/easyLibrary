...
import {BookElementsDto, CreateBookDto} from './dto/createBookDto';
...
export class BookService {
  ...
  async getUserBooks(userId: string): Promise<Book[]> {
    ...
    return this.bookModel.find({userId: userId}).select('-bookElements').exec();
  }
  ...
  async updateBook(id: string, updatedBookDto: CreateBookDto): Promise<Book> {
    const updatedBook =
        await this.bookModel.findByIdAndUpdate(id, updatedBookDto, {new: true})
            .exec();
    if (!updatedBook) {
      throw new NotFoundException(`Book with ID ${id} not found`);
    }
