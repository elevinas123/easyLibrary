...
export type SettingsType = {
  id: string; userId: string; fontSize: number; fontFamily: string;
  lineHeight: number;
...
export type ProcessedElement = {
  id: string; text: string; lineX: number; lineWidth: number; lineY: number;
  bookId: string;
  book?: Book;  // Optional reference
};
...
export type OffsetPosition = {
  id: string; x: number; y: number; bookId: string;
  book?: Book;  // Optional reference
};
...
export type ChaptersData = {
  ...
  href?: string;
  indentLevel?: number; bookId: string;
  book?: Book;  // Optional reference
};
...
export type CanvaElementSkeleton = {
  id: string; fill: string; x: number; y: number; width: number; height: number;
  strokeColor: string;
  strokeWidth: number;
  opacity: number;
...
export type CurveElementSkeleton = {
  id: string; points: Point[];  // Optional reference array
  fill: string;
  text?: string; roughness: number; bowing: number; seed: number;
  strokeWidth: number;
  strokeStyle: string;
  stroke: string;
  fillStyle: string;
...
export type ArrowElement = {
  id?: string;
  startId?: string;
  endId?: string; type: 'arrow';
  startType?: string;
  endType?: string;
  from?: CanvaElementSkeleton;  // Optional reference
  to?: CanvaElementSkeleton;    // Optional reference
  fromId?: string;
  toId?: string; curveId: string;
  curve?: CurveElementSkeleton;  // Optional reference
};

export type CircleElement = {
  id?: string; fillStyle: string; roughness: number; seed: number;
  hachureGap: number;
  hachureAngle: number;
  radius: number;
  type: 'circle';
...
export type TextElementType = {
  id?: string; text: string; fontFamily: string; fontSize: number; type: 'text';
  canvaId: string;
  canva?: CanvaElementSkeleton;  // Optional reference
};
